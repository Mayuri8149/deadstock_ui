...
export class ViewTransactionTypeComponent implements OnInit, OnDestroy {
	...
	companyCode=null
	companyName=null;
	filter:any={transactionTypeName:'',transactionTypeCode:'',moduleCode:'',moduleName:'',organizationName:'',companyCode:'',companyName:'',searchKey:''}
	...
	getTransType(recordPerPage, currentPage){
		...
		params = params.append('id', this.referenceId);
		if(this.role == 'sysadmin' || this.role == 'subadmin'){
	...
	goBack() {
		this.router.navigate(['/organizations/organizationModuleList'],{ queryParams: { currentPage: this.curPage, recordPerPage: this.orgPerPage}});
	}
