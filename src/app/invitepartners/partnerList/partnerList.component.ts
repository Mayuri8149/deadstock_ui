...
import { inviteModel } from 'src/app/modals/invite';
...
export class partnerListComponent implements OnInit, OnDestroy {
	...
	dataSource = new MatTableDataSource<inviteModel>();
	selection = new SelectionModel<inviteModel>(true, []);
	...
	entity=null;
	pstatus=null;
	filter:any={entity:'',entityId:'',entityName:'',email:'',pstatus:'',searchKey:''}
	curPage= 1;
	orgPerPage=5;
	pageIndex=0;
	pageSize=5
	...
	getInvitePartners(recordPerPage, currentPage,listActivited) {		
		...
		if(this.filter.entity){
			params = params.append('entity_Name', this.filter.entity);
		}
		...
		if(this.filter.pstatus){
			params = params.append('pstatus', this.filter.pstatus);
		}
	...
	approvedPartner(data) {
		...
		this.url = "/invitepartner/" + data.item._id + "/changeStatus";

		this.apiService.put(this.url, data.item)
			.subscribe((response) => {
				if(response.success == true) {
					if (data.status == 'Approved') {
	...
	viewSuperUsers(row) {
		if(this.curPage && this.orgPerPage){
			this.router.navigate(['/partners/superusers/'],{ queryParams: { id:row.childEntity,currentPage: this.curPage, recordPerPage: this.orgPerPage }} );
		}else{
			this.router.navigate(['/partners/superusers/'],{ queryParams: { id:row.childEntity,currentPage: this.currentPage, recordPerPage: this.recordPerPage }} );
		}
	}
		...
		if (row.corporate) {
			...
			this.subscriptions.push(this.apiService.get(this.url, params)
				...
				if(response.success == true){
					...
					if(this.curPage && this.orgPerPage){
						this.router.navigate(['/partner/partnerDetails'],{ queryParams: { id:this.id ,fromAdmin:true,partner:true,currentPage: this.curPage, recordPerPage: this.orgPerPage}} );
					}else{
						this.router.navigate(['/partner/partnerDetails'],{ queryParams: { id:this.id ,fromAdmin:true,partner:true,currentPage: this.currentPage, recordPerPage: this.recordPerPage}} );
					}
					}
				}
			}));
				...
				if (response.success == true) {
					...
					if (organizationData.isActive == false && this.loginUser.reference.role == 'admin') {
						...
						let details = {
							...
							organizationName: data.item.organization.name,
						}
						this.apiService.post(this.url, details)
							.subscribe((response: any) => {
								if (response.success == true) {
	...
	view(row) {
		...
		if(this.curPage && this.orgPerPage){
			this.router.navigate(['partners/partnerList/partnerUpdates/' + this.corpData._id + "/" + this.corpData.corporate.isBlockchainService],{ queryParams: { listActivited: this.listActivited,currentPage: this.curPage,recordPerPage: this.orgPerPage}} );
		}else{
			this.router.navigate(['partners/partnerList/partnerUpdates/' + this.corpData._id + "/" + this.corpData.corporate.isBlockchainService],{ queryParams: { listActivited: this.listActivited,currentPage: this.currentPage,recordPerPage: this.recordPerPage}});
		}
	}
