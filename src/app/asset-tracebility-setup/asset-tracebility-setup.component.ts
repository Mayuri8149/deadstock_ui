...
@Component({
	selector: 'app-asset-tracebility-setup.Component',
	templateUrl: './asset-tracebility-setup.component.html',
	styleUrls: ['./asset-tracebility-setup.component.css']
})
export class AssetTracebilitySetupComponent implements OnInit, OnDestroy {
	private subscriptions: Subscription[] = [];
	faHandPointLeft = faHandPointLeft;
	faSearch = faSearch;
	loggedInUser: any = {}
	assetTracebilityDetailForm: FormGroup;
	...
	ngOnInit(): void {
		...
		this.assetTracebilityDetailForm = this._formBuilder.group({
			...
			this.subscriptions.push(this.apiService.get(this.url, params)
				.subscribe((response: any) => {
					this.certtypes = response.data.result.transtypes.transtypes;
						...
						this.getassetTraceabilityData(certData);
				}))
		}
	...
	assetTracebility(data){		
		for (var i = 0; i < this.tempArray.length; i++) {
			...
			datas.isVerifiable = draft.isVerifiable,
			this.transData.push(datas);			
		}

		var obj = {
			assetTraceabilityField: this.transData,
			organizationId: this.selectedOrganization
		};
		this.url = "/assettracebility/create";
		this.subscriptions.push(this.apiService.post(this.url, obj)
		.subscribe((response: any) => {
			if(response.success == true){
				this.snackbarService.openSuccessBar('Record is added successfully.', "Asset Tracebility");
				this.getorganization();
				this.getUserLinkCertificateType(this.recordPerPage, this.currentPage);
				this.router.navigate(['/assetTracebilitySetup']);
			}
		}))
	}
	...
	changeRadioButton(row, event) {
		...
		} if (event.source.value == 'isVerifiable') {
		this.dataSource.data[index].isVerifiable = true
		}	
	}else{		
		if (event.source.value == 'isAssetName') {
			...
			this.dataSource.data[index].isVerifiable = false
		}
	}
	...
	getassetTraceabilityData(certData) {
		this.url = "/assettracebility/" + this.selectedOrganization
		var params = new HttpParams();
		this.subscriptions.push(this.apiService.get(this.url, params)
			.subscribe((response) => {
				if(response.success == true) {
					this.allorganizationData = response.data.result.assetTraceabilityField;
                        ...
                        for (var i = 0; i < this.allorganizationData.length; i++) {
						 ...
						 this.hashFields[this.allorganizationData[i]._id] = this.allorganizationData[i].fields;
							...
							if (index > -1) {								
								...
								if (this.allorganizationData[i].isVerifiable == true) {
									certData[index].isVerifiable = true
								}
							}
                        }						
	...
	filterByColumn() {
		this.moduleIdFilter.valueChanges.subscribe((moduleIdFilterValue) => {
		...
		this.labelFilter.valueChanges.subscribe((labelFilterValue) => {
